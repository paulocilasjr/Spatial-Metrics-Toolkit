...
cell_distribution_plot <- function(yaml_config=NULL){
  if(is.null(yaml_config$variables$sample_id) & 
    ...
    must be provided in yaml config file.", call.=FALSE)
  }
  ...
  data <- data[, c(sample_col, marker_set1)]
  plot_data <- matrix(0,nrow=nrow(data) * length(marker_set1),ncol=3)
  m <- 0
  for(i in marker_set1){
  ...
  plot_data[,3] <- gsub("..Positive.Cells", "",plot_data[,3])
  marker_set1 <- gsub("^X..", "",marker_set1)
  marker_set1 <- gsub("..Positive.Cells", "",marker_set1)
  colors <- createPalette(length(marker_set1), c("#2a6ebb", "#de3831", "#007367"), range = c(30, 80))
  names(colors) <- marker_set1
  theme_set(theme_bw())
  ggplot(plot_data, aes(y=as.numeric(plot_data[,2]), x= plot_data[,1], 
                        fill=factor(plot_data[,3], levels = marker_set1))) +
